...
offer_accepted = st.sidebar.selectbox("Offer Accepted", ["Yes", "No"])
reward = st.sidebar.selectbox("Reward", ["Air Miles", "Cash Back", "Points"])
income_level = st.sidebar.selectbox("Income Level", ["High", "Medium", "Low"])
...
credit_rating = st.sidebar.selectbox("Credit Rating", ["High", "Medium", "Low"])
...
if st.sidebar.button("Predict"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.success(f"The predicted average balance is: ${result['predicted_average_balance']:.2f}")
        else:
